(() => {
  ...
  const routeName = () => {
    try {
      if (
        frame.contentWindow &&
        frame.contentWindow.location.pathname.endsWith(
          "admin-arsip-detail.html",
        )
      )
        return "detail";
    } catch (e) {}
  ...
  document.querySelectorAll("[data-route]").forEach((link) => {
    ...
    toggle.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
      const open = group.classList.toggle("admin-nav-group--open");
      toggle.setAttribute("aria-expanded", String(open));
    };
    link.addEventListener("click", () => setTimeout(() => setOpen(route), 0));
  });
