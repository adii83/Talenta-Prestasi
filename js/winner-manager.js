...
function renderArchiveSources(){const root=document.getElementById('wmArchiveSourceList'),items=typeof getEffectiveArchivedCompetitions==='function'?getEffectiveArchivedCompetitions().filter(x=>x.active!==false):getArchivedCompetitions();root.innerHTML=`<p class="wm-source-note"><i data-lucide="database"></i> ${items.length} lomba Arsip tersedia; ${Math.min(wmDisplay.archiveLimit,items.length)} card akan tampil.</p>`+items.map((c,i)=>`<div class="wm-archive-source ${i<wmDisplay.archiveLimit?'is-included':''}"><i data-lucide="${esc(c.icon||'archive')}"></i><span><strong>${esc(c.name)}</strong><small>${(c.winnerCategories||[]).reduce((n,x)=>n+x.winners.filter(w=>w.active).length,0)} pemenang · Sumber Arsip</small></span><em>${i<wmDisplay.archiveLimit?'Ditampilkan':'Di luar batas'}</em></div>`).join('');icons()}
...
function archiveCards(){if(!wmDisplay.archiveActive)return'';const allItems=(typeof getPublicArchivedCompetitions==='function'?getPublicArchivedCompetitions():typeof getEffectiveArchivedCompetitions==='function'?getEffectiveArchivedCompetitions().filter(x=>x.active!==false):getArchivedCompetitions()).filter(c=>(c.winnerCategories||[]).some(cat=>cat.active!==false&&(cat.winners||[]).some(w=>w.active!==false)));const items=allItems.slice(0,wmDisplay.archiveLimit);if(!items.length)return'';return`<div class="archive-winners wm-preview__archives"><h3 class="archive-winners__title">${esc(wmDisplay.archiveTitle)}</h3><div class="grid grid--3">${items.map(c=>`<a href="arsip-detail.html?id=${encodeURIComponent(c.id)}#pemenang" class="lomba-card"><div class="lomba-card__thumb" style="background:${esc(c.gradient||'')}"><i data-lucide="${esc(c.icon||'archive')}"></i></div><div class="lomba-card__body"><h3 class="lomba-card__title">${esc(c.name)}</h3><p class="lomba-card__desc">${esc(c.description||'Lihat daftar pemenang ajang talenta sebelumnya.')}</p><span class="lomba-card__action">${esc(wmDisplay.archiveAction)} <i data-lucide="arrow-right"></i></span></div></a>`).join('')}</div></div>`}
