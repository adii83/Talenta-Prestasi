...
(() => {
  ...
  root.innerHTML = `<section class="lomba-banner" style="background:${esc(competition.gradient)}"><div class="lomba-banner__content"><h1 class="lomba-banner__title">${esc(competition.name)}</h1><p class="lomba-banner__desc">${esc(competition.description || "")}</p></div></section><nav class="archive-detail-breadcrumb" aria-label="Breadcrumb"><div class="container"><p class="t-caption"><a href="arsip.html">Arsip</a><span aria-hidden="true">/</span><span>${esc(competition.name)}</span></p></div></nav>${winnersSection}${documentsSection}`;
  lucide.createIcons();
  if (location.hash)
