...
export class PublicService {
  ...
  async bootstrap(hostname: string, previewToken = '') {
    ...
    return response(resolved.snapshot.bootstrap, resolved.preview);
  }
  ...
  async winners(categorySlug: string, previewToken = '') {
    const resolved = await this.resolve(categorySlug, previewToken);
    const base = resolved.snapshot.winners as Record<string, unknown>;
    ...
    return response(
      {
        ...base,
        archives: archives
          .map((item) => archiveSummary(item.snapshot))
    ...
    return response(
      {
        ...
        events: events.map((item) => archiveSummary(item.snapshot)),
      },
      resolved.preview,
    );
  }
  ...
  async archiveDetail(
    ...
    const resolved = await this.resolve(categorySlug, previewToken);
    const rows = await this.db.query<Array<{ snapshot: PublicEventSnapshot }>>(
      `SELECT publication.public_snapshot AS snapshot
  ...
  private archiveSnapshots(categoryId: string, activeEventId: string) {
    return this.db.query<Array<{ snapshot: PublicEventSnapshot }>>(
      `SELECT publication.public_snapshot AS snapshot
       FROM event_sites event
       JOIN event_publications publication ON publication.event_site_id=event.id
...
function archiveSummary(snapshot: PublicEventSnapshot) {
  return (snapshot.archiveDetail as { event: unknown }).event;
}

const PUBLISHED_SELECT = `SELECT event.id AS "eventId",category.id AS "categoryId",
  publication.public_snapshot AS snapshot
