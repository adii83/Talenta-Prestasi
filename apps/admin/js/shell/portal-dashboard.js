(() => {
  ...
  function ensureUi() {
    ...
    style.textContent = `
      ...
      .event-dashboard__header{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:20px clamp(24px,6vw,80px);background:#0a2b50;color:#fff}.event-dashboard__brand{font-weight:800}.event-dashboard__brand small{display:block;color:#cbd9e9;font-size:.75rem;font-weight:500;letter-spacing:.04em}.event-dashboard__actions{display:flex;align-items:center;gap:12px}.event-dashboard__email{color:#cbd9e9}
    ...
    root.hidden = true;
    root.innerHTML = `
      <header class="event-dashboard__header"><div class="event-dashboard__brand">Talenta Prestasi<small>Admin Editor</small></div><div class="event-dashboard__actions"><span class="event-dashboard__email" id="eventUserEmail"></span><button class="event-dashboard__logout" id="eventLogout" type="button">Keluar</button></div></header>
  ...
  function latestTemplateEvent() {
    return [...events].sort(
      (left, right) =>
        (right.periodYear ?? -Infinity) - (left.periodYear ?? -Infinity) ||
        (right.batchNumber ?? 1) - (left.batchNumber ?? 1) ||
  ...
  function openCreate() {
    ...
    if (selectedCategory) {
      ...
      const template = latestTemplateEvent();
      document.getElementById("eventTemplateFields").hidden = !template;
      document.getElementById("eventTemplateSource").textContent = template
        ? `Sumber: ${template.name} · ${periodLabel(template)}`
        : "";
      syncBatchFields();
    }
    ...
    await submit(
      ...
      async (formBody) => {
        const body = {
          ...
          useLatestTemplate: formBody.useLatestTemplate === "on",
        };
        try {
          await TalentaApi.request(
  ...
  function addSwitchButton() {
    ...
    link.innerHTML = `<i data-lucide="arrow-left"></i>Kategori & Event`;
    document.getElementById("adminSidebar")?.append(link);
    if (window.lucide) lucide.createIcons({ nodes: [link] });
  }
